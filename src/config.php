...
$db_host = (string)entorno_valor('DB_HOST', 'localhost');
$db_port = (string)entorno_valor('DB_PORT', '3306');
$db_name = (string)entorno_valor('DB_NAME', 'ticketia');
$db_user = (string)entorno_valor('DB_USER', 'ticketia');
$db_pass = (string)entorno_valor('DB_PASS', '');
...
$llm_config = [
    'connect_timeout' => (int)entorno_valor('LLM_CONNECT_TIMEOUT', 5),
    'timeout_chat' => (int)entorno_valor('LLM_TIMEOUT', 120),
    'timeout_traductor' => (int)entorno_valor('LLM_TIMEOUT_TRADUCTOR', 60),
];
...
$llm_local_endpoint = (string)entorno_valor('LLM_LOCAL_ENDPOINT', 'http://localhost:1234/v1/chat/completions');
$llm_local_model = (string)entorno_valor('LLM_LOCAL_MODEL', '');
if ($llm_local_endpoint !== '') {
    $llm_config['local'] = [
        'label' => 'IA local',
        'endpoint' => $llm_local_endpoint,
        'api_key' => entorno_valor('LLM_LOCAL_API_KEY'),
...
$llm_solo_local = entorno_valor('LLM_SOLO_LOCAL', '') === '1';

if (!$llm_solo_local && (string)entorno_valor('OPENAI_API_KEY', '') !== '') {
    $llm_config['openai'] = [
        'label' => 'OpenAI',
        'endpoint' => 'https://api.openai.com/v1/chat/completions',
        'api_key' => (string)entorno_valor('OPENAI_API_KEY'),
        'chat' => ['model' => (string)entorno_valor('OPENAI_MODEL', 'gpt-5'), 'token_param' => 'max_completion_tokens', 'max_tokens' => 5000, 'temperature' => null],
        'stream' => ['model' => (string)entorno_valor('OPENAI_MODEL_STREAM', 'gpt-4.1-mini'), 'token_param' => 'max_tokens', 'max_tokens' => 10000, 'temperature' => 0.1],
        'traductor' => ['model' => (string)entorno_valor('OPENAI_MODEL', 'gpt-5'), 'token_param' => 'max_completion_tokens', 'max_tokens' => 5000, 'temperature' => null],
    ];
}

if (!$llm_solo_local && (string)entorno_valor('XAI_API_KEY', '') !== '') {
    $llm_config['xai'] = [
        'label' => 'xAI',
        'endpoint' => 'https://api.x.ai/v1/chat/completions',
        'api_key' => (string)entorno_valor('XAI_API_KEY'),
        'chat' => ['model' => (string)entorno_valor('XAI_MODEL', 'grok-3'), 'token_param' => 'max_tokens', 'max_tokens' => 5000, 'temperature' => 0.1],
        'stream' => ['model' => (string)entorno_valor('XAI_MODEL_STREAM', 'grok-3-mini-beta'), 'token_param' => 'max_tokens', 'max_tokens' => 10000, 'temperature' => 0.1],
        'traductor' => ['model' => (string)entorno_valor('XAI_MODEL', 'grok-3'), 'token_param' => 'max_tokens', 'max_tokens' => 5000, 'temperature' => 0.1],
    ];
}
