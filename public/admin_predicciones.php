...
<section class="admin-panel"><h2>Estado del historico</h2><p>Distribucion actual de todas las incidencias de esta organizacion, incluidas las archivadas.</p>
    <?php $m = $metricas[$id] ?? []; $estados_grafico = ['Abiertas'=>(int)($m['abiertas'] ?? 0), 'En curso'=>(int)($m['en_curso'] ?? 0), 'Resueltas'=>(int)($m['resueltas'] ?? 0), 'Cerradas'=>(int)($m['cerradas'] ?? 0)]; $maximo = max(1, ...array_values($estados_grafico)); ?>
