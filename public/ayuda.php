...
<?php if (conocimiento_valoraciones_disponibles($pdo)): ?><section class="admin-panel"><h2>Te ha servido esta guia?</h2><?php if (isset($_GET['valorado'])): ?><p role="status">Gracias. Usaremos tu valoracion para mejorar las soluciones.</p><?php endif; ?><form method="POST" action="valorar_articulo.php" class="page-tools"><?= csrf_campo() ?><input type="hidden" name="id" value="<?= $id ?>"><button class="card-button secondary-button" name="util" value="1">Si, me ha ayudado</button><button class="card-button secondary-button" name="util" value="0">No he podido resolverlo</button></form></section><?php endif; ?>
<?php endif; ?>
<?php else: ?>
