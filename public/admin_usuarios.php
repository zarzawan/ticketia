                ...
                try {
                    ...
                    if (!in_array($rol, ['admin', 'operador'], true) || !$activo) {
                        $desasignadas = $pdo->prepare(
                            "UPDATE incidencias SET asignado_id = NULL WHERE asignado_id = :id AND estado IN ('abierta','en_curso','esperando_cliente')"
                        );
...
$sql = "SELECT u.*, c.nombre AS empresa,
               (SELECT COUNT(*) FROM incidencias i WHERE i.asignado_id = u.id AND i.estado IN ('abierta','en_curso','esperando_cliente')) AS tickets_abiertos
        FROM usuarios u
