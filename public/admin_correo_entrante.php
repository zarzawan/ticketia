...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    try {
        ...
        if (($_POST['accion'] ?? '')==='importar') {
            ...
            $ticket=gmail_importar($pdo,$id,(int)auth_usuario()['id']);$aviso='Incidencia #' . $ticket . ' creada y encolada para clasificacion IA.';
...
<?php if($detalle['estado']==='pendiente'):?><form method="POST" class="form-stack"><?= csrf_campo() ?><input type="hidden" name="id" value="<?= (int)$detalle['id'] ?>"><label><input type="checkbox" name="confirmar"> He verificado la identidad del remitente, su organizacion y el contenido. Autorizo crear la incidencia y clasificarla con el proveedor IA configurado.</label><div class="page-tools"><button name="accion" value="importar" class="card-button">Crear incidencia con IA</button><button name="accion" value="descartar" class="card-button secondary-button">Descartar de la revision</button></div></form><?php endif; ?></section><?php endif; ?>
